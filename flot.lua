...
function flot.Plot (opts)
  ...
  plot.doctitle = opts.doctitle
  plot.filename = opts.filename
...
function flot.render (plot)
  local p = flot.path
  local title = plot.doctitle
  local name = plot.filename
  if arg and not name then -- not interactive
    name = arg[0]:match('([%w_%-]+)%.lua')
  end
  name = name:gsub("%s+", ''):lower()
  if not title then title = name end

  local header = prefix:format(title,p,p,p)
